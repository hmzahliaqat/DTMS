        ...
        <ul class="nav navbar-nav align-items-center ml-auto">
            ...
            <li class="nav-item dropdown dropdown-user"><a class="nav-link dropdown-toggle dropdown-user-link" id="dropdown-user" href="javascript:void(0);" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                    <div class="user-nav d-sm-flex d-none"><span class="user-name font-weight-bolder">{{ auth()->user()->name }}</span>
                     ...
                     @role('student')
                    <span class="user-status">Student</span></div><span class="avatar">
                        @endrole

                        @role('hod_cs')
                        <span class="user-status">HOD CS</span></div><span class="avatar">
                            @endrole
                            @role('hod_eed')
                            <span class="user-status">HOD EE</span></div><span class="avatar">
                                @endrole
                                @role('hod_mth')
                                <span class="user-status">HOD Maths</span></div><span class="avatar">
                                    @endrole
                                    @role('faculty')
                                    <span class="user-status">Faculty</span></div><span class="avatar">
                                        @endrole

                            @if (auth()->user()->profile_photo_path != null)

                        <img class="round" src="{{ asset('/storage/'. auth()->user()->profile_photo_path ) }}" alt="avatar" height="40" width="40"><span class="avatar-status-online"></span></span>

                        @else
                        <img class="round" src="{{ asset('/storage/profile-photos/no-profile-picture-icon.png' ) }}" alt="avatar" height="40" width="40"><span class="avatar-status-online"></span></span>

                        @endif

                    </a>
                <div class="dropdown-menu dropdown-menu-right" aria-labelledby="dropdown-user"><a class="dropdown-item" href="{{ route('profile.show') }}"><i class="mr-50" data-feather="user"></i> Profile</a><a class="dropdown-item" href="app-email.html"><i class="mr-50" data-feather="mail"></i> Inbox</a><a class="dropdown-item" href="app-todo.html"><i class="mr-50" data-feather="check-square"></i> Task</a><a class="dropdown-item" href="app-chat.html"><i class="mr-50" data-feather="message-square"></i> Chats</a>
